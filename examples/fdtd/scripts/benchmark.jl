...
function max_perf_benchmark(exe, variant, n_cus, f, loop_latency)
    ...
    raw_metrics = build_metrics(kernel_runtime, n_timesteps * N_SUBGENERATIONS, variant, f, loop_latency, grid_wh, grid_wh, TILE_SIZE, TILE_SIZE, n_cus, OPERATIONS_PER_CELL, CELL_SIZE)
        ...
        for rel_width in 0.1:0.1:1.0
            for t_max in 1.0:1.0:30.0
                ...
                if variant == :monotile
                    ...
                    model_runtime = model_tiling_runtime(f, loop_latency, grid_wh, grid_wh, N_SUBGENERATIONS * n_timesteps, TILE_SIZE, TILE_SIZE, N_TILING_CUS)
                end
