...
struct HeatKernel : public BaseTransitionFunction
{
    ...
    double operator()(Stencil<double, 1> const &stencil) const
    {
        ...
        else
        {
            ...
            // Calculation
            return center + a * dt * ((left - 2.0 * center + right) / (DX * DX) + (top - 2.0 * center + bottom) / (DY * DY));
        }
    }
};
...
int main(int argc, char **argv)
{
    ...
    const uindex_t rows = stoi(argv[1]);
    const uindex_t cols = stoi(argv[2]);
    uindex_t nsteps = stoi(argv[3]);
